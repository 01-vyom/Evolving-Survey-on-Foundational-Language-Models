...
# torch.backends.cuda.matmul.allow_tf32 = False # set this for older versions of PyTorch
# torch.cuda.empty_cache()
# torch.cuda.set_per_process_memory_fraction(0.5, 0)
# torch.cuda.set_allocator(torch.cuda.MaxMemoryAllocator(max_split_size_mb))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Finetune a transformers model on SuperGLUE")
    parser.add_argument(
        "--task_name",
        type=str,
        default=None,
    ...
    parser.add_argument("--clip", type=float, default=0.0, help="Gradient Clipping to use.")
...
def main():
    ...
    # send_example_telemetry("run_"+args.dataset_name+"_no_trainer", args)

    # Initialize the accelerator. We will let the accelerator handle device placement for us in this example.
    # If we're using tracking, we also need to initialize it here and it will by default pick up all supported trackers
    # in the environment
    accelerator = (
        Accelerator(log_with=args.report_to, logging_dir=args.output_dir) if args.with_tracking else Accelerator("cuda")
    )
    ...
    if ("gpt" in args.model_name_or_path) or ("transfo-xl" in args.model_name_or_path):
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers(args.dataset_name + "_no_trainer", experiment_config)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.with_tracking:
            accelerator.log(
                {
                    "accuracy" if args.task_name is not None else args.dataset_name: eval_metric,
        ...
        if args.checkpointing_steps == "epoch":
            ...
            if best_metric == []:
                ...
                best_metrics = list(best_metric.values())
                curr_metrics = list(eval_metric.values())
                save = True
                for i in range(len(curr_metrics)):
                    if curr_metrics[i] < best_metrics[i]:
                        save = False
                if save:
                    model.save_pretrained(
                    args.output_dir, is_main_process=accelerator.is_main_process, save_function=accelerator.save
